function build_ieee39_sg5_dfig5_layout_optimized_v07()
%build_ieee39_sg5_dfig5_layout_optimized_v07 Build readable v0.7 wrapper.
%
...
targetModel = "ieee39_sg5_dfig5_layout_optimized_v07";
...
exportPreview(targetModel, fullfile(projectRoot, "build", "reports", "ieee39_layout_optimized_v07_top.png"));
exportPreview(overviewPath, fullfile(projectRoot, "build", "reports", "ieee39_layout_optimized_v07_overview.png"));
exportPreview(regionalPath + "/Zone_Central_Overview", fullfile(projectRoot, "build", "reports", "ieee39_layout_optimized_v07_zone_central.png"));
...
fprintf("Layout optimized report: %s\n", fullfile(projectRoot, "build", "reports", "layout_optimized_v07_report.md"));
end
...
for i = 1:size(mpc.branch, 1)
    ...
    [pos, orientation] = zoneBranchPlacement(busXY(fromBus), busXY(toBus), i);
    path = scope + "/" + sprintf("L%03d_%03d_%02d", fromBus, toBus, i);
    add_block("built-in/Subsystem", path, ...
        "Position", pos, ...
        "Orientation", orientation, ...
...
for busId = zone.buses
    ...
    path = sprintf("%s/B%03d", scope, busId);
    add_block("built-in/Subsystem", path, ...
        "Position", centeredBox(xy(1), xy(2), 34, 52), ...
        "BackgroundColor", "lightBlue", ...
        "ShowName", "off");
    Simulink.SubSystem.deleteContents(path);
    set_param(path, "AttributesFormatString", sprintf("%d", busId));
...
try
    ann = Simulink.Annotation(scope, sprintf([ ...
        ...
        "Layout report: build/reports/layout_optimized_v07_report.md\n" ...
...
try
    ann = Simulink.Annotation(modelName, "IEEE39 SG/DFIG layout-optimized model v0.7");
...
sub = raw(idx, :);
x = normalizeToRange(sub(:, 2), 135, 615);
y = normalizeToRange(sub(:, 3), 135, 385);
busXY = containers.Map("KeyType", "double", "ValueType", "any");
for i = 1:size(sub, 1)
...
function [pos, orientation] = zoneBranchPlacement(a, b, index)
v = b - a;
...
if abs(v(1)) >= 1.15 * abs(v(2))
    orientation = "right";
    len = clamp(abs(v(1)) - 70, minLength, maxLength);
    pos = centeredBox(mid(1), mid(2), len, thick);
elseif abs(v(2)) >= 1.15 * abs(v(1))
    orientation = "down";
    len = clamp(abs(v(2)) - 70, minLength, maxLength);
    pos = centeredBox(mid(1), mid(2), thick, len);
else
    orientation = "right";
    len = clamp(max(abs(v)) * 0.55, minLength, maxLength);
    pos = centeredBox(mid(1), mid(2), len, thick);
end
end
...
reportFile = fullfile(projectRoot, "build", "reports", "layout_optimized_v07_report.md");
fid = fopen(reportFile, "w", "n", "UTF-8");
cleanup = onCleanup(@() fclose(fid));
fprintf(fid, "# IEEE39 Layout-Optimized v0.7 Report\n\n");
fprintf(fid, "Model: `%s`\n\n", modelFile);
fprintf(fid, "## Structure\n\n");
fprintf(fid, "- `Topology_Overview`: clean IEEE39 one-line review view.\n");
fprintf(fid, "- `Area_Partitioned_Physical_Detail`: executable v0.6 area-partitioned SPS network retained for simulation.\n");
...
fprintf(fid, "- v0.7 keeps the root and regional submodules as navigation/review layers and keeps executable physical wiring in the physical detail layer.\n");
...
fprintf(fid, "  - `build/reports/ieee39_layout_optimized_v07_top.png`\n");
fprintf(fid, "  - `build/reports/ieee39_layout_optimized_v07_overview.png`\n");
fprintf(fid, "  - `build/reports/ieee39_layout_optimized_v07_zone_central.png`\n");
end
