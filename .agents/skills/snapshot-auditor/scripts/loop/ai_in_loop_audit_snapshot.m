...
if nargin < 3 || strlength(string(snapshotDir)) == 0
    ...
    if strlength(string(snapshotRoot)) == 0
        error('AIInLoop:SnapshotAuditMissingRoot', ...
            'Pass snapshotDir explicitly or set AI_SUMMARY_ROOT.');
    end
    snapshotDir = fullfile(snapshotRoot, char(modelName));
end
...
envRoot = getenv('SIMULINK_AGENT_ROOT');
