...
## Three Status Axes (do not collapse them)

A plan reports three independent results; passing the contract is NOT solver
readiness:

- `contract_status` (`pass`/`provisional`/`fail`): is the plan self-consistent
  and admissible? This is all the metadata can establish on its own.
- `model_validation_status` (`not_attempted`/`pass`/`fail`): did a real
  `load`/`update`/`simulate` back it? Set only by an attached `ModelProbe`.
- `handoff_ready` (logical): true only when contract passes, the model probe
  passes, and there are zero warnings.

`verified_against_model` is DERIVED from probe evidence, never asserted: a bare
`true` claim with no probe is downgraded to a warning and stays false. Hardware
(HIL) validation is a further, separate axis this skill does not assert.
...
evidence. On its own it can only set `contract_status`.

## Model-Backed Probe

To move from contract-consistency to model-backed evidence, build and run the
tiny non-private probe, then attach its result as `ModelProbe`:

```matlab
addpath(".agents/skills/hybrid-solver-multirate-simulation/probe")
probe = build_and_run_tiny_multirate_probe( ...
    "OutputDir", "build/reports/m1_multirate_solver/model_probe", "StopTime", 0.05);
summary = summarize_multirate_solver_plan(plan, "ModelProbe", probe, ...
    "OutputDir", "build/reports/m1_multirate_solver/model_backed");
```

The probe builds a genuine multirate model from scratch (continuous plant + a
fast and a slow discrete rate + rate transitions), runs `load`/`update`/
`simulate`, and returns evidence. It never opens or copies a private or lab
model, and the `.slx` lives only under the gitignored `build/` tree. A passing
probe sets `model_validation_status=pass` and derives `verified_against_model`;
warnings still block `handoff_ready`.
...
The markdown leads with all three axes (contract status, model validation,
handoff ready) so a contract pass can never be mistaken for solver readiness.
