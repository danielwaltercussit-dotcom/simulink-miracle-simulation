...
- `verified_against_model`: this is a CLAIM only and is never trusted on its
  own. The reported `verified_against_model` is DERIVED from attached model
  evidence (see Three Status Axes). A bare `true` claim with no probe is
  downgraded to a warning and the derived value stays `false`.
...
## Three Status Axes (never collapse into one headline)

Contract consistency, model-backed validation, and hardware validation are
SEPARATE. A documented, admissible plan is not solver-validated, and a
software model run is not hardware validation.

- `contract_status` in {`pass`, `provisional`, `fail`}: is the plan itself
  self-consistent and admissible against the supplied anchors? This is the only
  axis the plan metadata can satisfy on its own.
- `model_validation_status` in {`not_attempted`, `pass`, `fail`}: did a real
  `load`/`update`/`simulate` back this plan? Driven ONLY by an attached
  `ModelProbe` evidence struct (`.ran`, `.sim_success`, finite `.max_abs_state`,
  `.model`). With no probe it is `not_attempted`, regardless of any claim.
- `handoff_ready` (logical): the gate. True only when
  `contract_status == pass` AND `model_validation_status == pass` AND there are
  zero warnings. Warnings (under-sampled slow mode, non-integer ratio, stiff
  solver, unbacked verification claim) all block handoff readiness even when the
  model run passed.

Hardware-in-the-loop validation is a further, separate axis this skill does NOT
assert; route real-time/HIL feasibility to `hil-readiness-real-time-prep` and
never let a software probe read as hardware evidence.

## Model-Backed Probe

To move from contract-consistency to model-backed evidence, build and run a
tiny, NON-PRIVATE model and attach the result:

```matlab
addpath(".agents/skills/hybrid-solver-multirate-simulation/probe")
probe = build_and_run_tiny_multirate_probe( ...
    "OutputDir", "build/reports/m1_multirate_solver/model_probe", ...
    "StopTime", 0.05);
summary = summarize_multirate_solver_plan(plan, "ModelProbe", probe, ...
    "OutputDir", "build/reports/m1_multirate_solver/<case>");
```

The probe builds a genuine multirate model from scratch (continuous plant +
fast/slow discrete rates + rate transitions), runs `load`/`update`/`simulate`,
and returns evidence. It never opens or copies a private or lab model, and the
generated `.slx` is written only under the gitignored `build/` tree.
...
## Failure vs Warning vs Provisional (contract_status axis)
...
- `failure`: an impossible or unresolvable choice — step out of `(0, stop_time)`,
  ...
  `contract_status = "fail"`. A failed MODEL probe is reported separately as
  `model_validation_status = "fail"` and does NOT change `contract_status`.
- `warning`: an admissible-but-risky choice — under-sampled slow mode,
  non-integer rate ratio, high stiffness ratio under a non-stiff solver, or a
  bare verification claim with no probe. Warnings do not fail the contract but
  DO block `handoff_ready`.
...
- A `contract_status = "pass"` plan means the step/solver choices are
  *self-consistent and admissible against the supplied anchors*. It does not
  prove the model is numerically converged and is NOT solver readiness; only a
  passing model probe (`model_validation_status = "pass"`) plus zero warnings
  makes `handoff_ready` true, and even that is software, not hardware, evidence.
...
- `verified_against_model` is DERIVED, not asserted: it is true only when a
  `ModelProbe` with `ran=true`, `sim_success=true`, and finite `max_abs_state`
  is attached. A bare `verified_against_model=true` claim with no probe is
  downgraded to a warning and the derived value stays false. Plan-only evidence
  must never claim model verification.
- Do not convert a large or private model to multirate form to satisfy this
  plan. Use the tiny non-private probe or synthetic anchors.
