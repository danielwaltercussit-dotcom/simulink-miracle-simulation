...
S9 REPORT           → Markdown + status.json + screenshots
```
...
| S6 | `simulink-power-electronics` (PE) + project tuning scripts | `simulink-solver-profiler-analyzer` if numeric issue; `matlab-optimize-performance` for slow tuning scripts |
| S7 | `simulink-model-verification`, `testing-simulink-models` | `filing-bug-reports` if real defect found; `matlab-write-performance-tests` for `matlab.perftest` regressions in `scripts/loop/`; mathworks-ci-verify reference: `LaneFollowingExecModelAdvisor.m` pattern for ModelAdvisor harness |
| S8 | this skill (router) | `simulink-profiler-analyzer` for perf, `code-simplifier` / `matlab-modernize-code` for script cleanups |
| S9 | this skill | — |
...
- S7 writes/runs `tests/ai_in_loop_functional_model_test.m` and delegates to
  `scripts/verification/verify_power_system_model.m` when no Simulink Test
  `.mldatx` harness is available, then emits `sltest_summary.md` and
  `model_verification_summary.md`.
- S9 re-reads `status.json`, verifies `update/smoke/tune` booleans, checks
  required artifacts, and ensures `top.png` exists.
